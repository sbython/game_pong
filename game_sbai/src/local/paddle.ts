import { Mesh, MeshBuilder, Scene, StandardMaterial, Color3, Vector3, ScenePerformancePriority  } from '@babylonjs/core';
...
class Paddle {     
    ...
    constructor(scene: Scene, position: Vector3, usernameText: string) {
        this.scene = scene;
    ...
    public createUserName(x:number) {
        this.username = MeshBuilder.CreatePlane('username', {width: 3, height: 2}, this.scene);
        if (x < 0) {
        ...
        this.textBlock.textHorizontalAlignment = TextBlock.HORIZONTAL_ALIGNMENT_CENTER;
        this.textBlock.text = this.usernameText;
        this.textBlock.color = "white";
        this.textBlock.fontSize = 400; // Use a large font size for sharpness
        this.textBlock.textWrapping = true; // Allows text to wrap within the plane's boundaries 
        TEXT.addControl(this.textBlock); // Add the control to the texture
    }
